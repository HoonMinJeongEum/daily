...
export function addCoupon(token, body) {
  const res = http.post(`${BASE}/coupons`, JSON.stringify(body), {
...
export function deleteCoupon(token, couponId) {
  const res = http.del(http.url`${BASE}/coupons/${couponId}`, null, {
    headers: jsonHeaders(token),
  });

  check(res, {
    "delete 204/2xx": (r) =>
      r.status === 204 || (r.status >= 200 && r.status < 300),
  });
  return res;
}

export function buyCoupon(profileToken, body) {
  const res = http.post(`${BASE}/coupons/buy`, JSON.stringify(body), {
...
export function useCoupon(profileToken, body) {
  const res = http.patch(`${BASE}/coupons/use`, JSON.stringify(body), {
    headers: jsonHeaders(profileToken),
  });
  check(res, {
    "use 204/2xx": (r) =>
...
export function getCoupons(profileToken) {
  const res = http.get(`${BASE}/coupons`, {
    headers: jsonHeaders(profileToken),
  });
...
export function getCouponsChild(token) {
  const res = http.get(`${BASE}/coupons/child`, {
    headers: jsonHeaders(token),
  });
...
export function getCouponsUser(profileToken) {
  const res = http.get(`${BASE}/coupons/user`, {
    headers: jsonHeaders(profileToken),
  });
...
export function insertAndBuyCoupons(token) {
  const res = http.get(`${BASE}/test/insertAndBuy`, {
    headers: jsonHeaders(token),
  });
  check(res, { "insert 2xx": (r) => r.status >= 200 && r.status < 300 });
  return res;
